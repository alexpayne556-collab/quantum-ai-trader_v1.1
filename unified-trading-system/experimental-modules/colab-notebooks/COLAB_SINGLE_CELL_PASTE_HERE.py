...
def create_target(df):
    """BUY/HOLD/SELL classification using cross-sectional ranking"""
    df = df.copy()
    ...
    df['ret_rank'] = df.groupby('date')['fwd_ret'].rank(pct=True) * 100
    
    # 3 classes: SELL (0-33%), HOLD (33-67%), BUY (67-100%)
    df['target'] = pd.cut(df['ret_rank'], bins=[0, 33, 67, 100], labels=[0, 1, 2], include_lowest=True).astype(int)
